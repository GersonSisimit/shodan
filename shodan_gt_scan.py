...
#!/usr/bin/env python3
"""
Shodan Guatemala Console Scanner
--------------------------------
Requisitos de la tarea:
  - Usar la API de Shodan.
  - Ejecutar una consulta dirigida a Guatemala (se agrega automáticamente country:GT).
  - Prohibido usar filtro por organización (org:). El script lo valida.
  - Mostrar TODOS los resultados obtenidos (de las páginas que se consulten) en consola.
  - Generar un resumen final con:
      * Total de direcciones IP identificadas (únicas).
      * Total de IPs por puerto abierto.
  - Incluir en la salida: número de carnet, nombre completo, curso y sección.

Uso básico:
  python shodan_gt_scan.py --api-key SHODAN_KEY --query 'city:"Jalapa"' \

                           --carnet 20210000 --nombre "Nombre Apellido" \

                           --curso "Curso" --seccion "A" --pages 1 --page-size 100

Nota: si no pasas --api-key, intentará leerla de la variable de entorno SHODAN_API_KEY.
"""
...
def obtener_api_key(cli_key: str | None) -> str:
    key = "6dmkPi7m2rlgdzwJ0HYgCC0CBXVyV30Y"
    # key = cli_key or os.getenv("SHODAN_API_KEY")
    # if not key:
    #     raise ValueError("Falta API key. Pásala con --api-key o variable de entorno SHODAN_API_KEY.")
    return key
